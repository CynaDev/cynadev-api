...
class CheckoutConfirmController extends AbstractController
{
    ...
    public function __invoke(string $sessionId): JsonResponse
    {
        ...
        $emailSent = $this->emailService->sendOrderConfirmationEmail($user, $order, $cartItems);

        return new JsonResponse(['status' => 'created', 'orderId' => $order->getId(), 'emailSent' => $emailSent]);
    }
}
